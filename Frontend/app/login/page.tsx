...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        alert("Selamat Datang Kembali di BAKMIE WARUWU, " + data.username + "!");
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-gray-50 px-6 py-20 relative overflow-hidden">
      ...
      <div className="absolute top-[-10%] right-[-10%] w-96 h-96 bg-red-700/5 rounded-full blur-[100px] -z-10"></div>
      <div className="absolute bottom-[-10%] left-[-10%] w-96 h-96 bg-yellow-400/10 rounded-full blur-[100px] -z-10"></div>
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-10">
          ...
          <h2 className="text-xl font-bold text-gray-800 mt-6 uppercase tracking-widest">Login Pelanggan</h2>
          <p className="text-gray-400 text-xs mt-2 font-medium">Masuk untuk menikmati bakmie legendaris favoritmu</p>
        </div>
        ...
        <div className="bg-white p-8 md:p-10 rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-gray-100 backdrop-blur-sm">
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <button 
              type="submit" 
              disabled={loading}
              className={`w-full py-5 rounded-2xl font-black text-[11px] uppercase tracking-[0.3em] shadow-xl transition-all active:scale-95 flex justify-center items-center ${
                loading 
          ...
          <div className="mt-10 text-center border-t border-gray-50 pt-8">
            <p className="text-xs text-gray-500 font-medium">
              Belum punya akun? {" "}
              <Link href="/register" className="text-red-700 font-black hover:text-yellow-500 transition-colors underline-offset-4 decoration-2">
                DAFTAR GRATIS
              </Link>
            </p>
          </div>
