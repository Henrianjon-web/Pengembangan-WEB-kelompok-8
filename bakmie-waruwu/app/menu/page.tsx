...
// 2. Gunakan 'export default function' untuk memperbaiki error 500
export default function MenuPage() {
  ...
  // Pisahkan kategori sesuai database
    ...
    <main className="min-h-screen bg-white pt-28 pb-20">
      <div className="container mx-auto px-6">
        ...
        <section className="mb-24">
          ...
          {loading ? (
            <div className="text-center text-gray-400">Memuat Makanan...</div>
          ) : (
        ...
        <section>
          ...
          {loading ? (
            <div className="text-center text-gray-400">Memuat Minuman...</div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {minuman.map((item) => (
                <MenuCard key={item.id} item={item} addToCart={addToCart} />
...
// Komponen Card (Tanpa export default karena ini sub-komponen)
function MenuCard({ item, addToCart }: { item: Product, addToCart: any }) {
  return (
    <div className="group bg-white rounded-[2rem] shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-100 overflow-hidden flex flex-col p-4">
      <div className="h-48 bg-gray-100 relative overflow-hidden rounded-[1.5rem]">
        ...
        />
        {item.is_best_seller && (
          <div className="absolute top-3 left-3 bg-yellow-400 text-red-900 px-3 py-1 rounded-full text-[8px] font-black">
            ⭐ BEST SELLER
          </div>
        )}
      </div>
      <div className="p-4 flex flex-col flex-grow text-black">
        ...
        <p className="text-red-600 font-black text-xl mt-2 italic">
          Rp{item.harga.toLocaleString('id-ID')}
        </p>
        <button 
          onClick={() => addToCart(item)}
          className="mt-4 w-full bg-gray-900 text-white py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-red-700 transition-all"
        >
          + Keranjang
        </button>
      </div>
